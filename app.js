document.addEventListener('DOMContentLoaded', async () => {
    ...
    const dict = {
        ...
        ja: {
            ...
            analysis: "分析", trend: "7日間の推移", recorded: "記録済み",
    ...
    const mentalIcons = ['<i class="far fa-sad-tear"></i>', '<i class="far fa-frown"></i>', '<i class="far fa-meh"></i>', '<i class="far fa-smile"></i>', '<i class="far fa-laugh-beam"></i>'];
    ...
    if (btnGuideOpen && guideModal && settingsModal) {
        btnGuideOpen.addEventListener('click', () => { settingsModal.style.display = 'none'; guideModal.style.display = 'flex'; });
    }
    if (btnGuideClose && guideModal && settingsModal) {
        btnGuideClose.addEventListener('click', () => { guideModal.style.display = 'none'; settingsModal.style.display = 'flex'; });
    }
    ...
    const kpiAdvice = {
        ja: {
            ...
            fat_p: "体脂肪率は水分量に影響されます。毎日同じ条件で測定し、長期的な傾向を追うことが重要です。",
            calories: "AIが算出した本日の推定摂取カロリーです。目標範囲内に収めることで計画的なコントロールが可能です。",
            sleep_h: "睡眠はリカバリーの最重要ファクターです。7時間前後を確保することでメンタルと体重の安定に直結します。",
            mental: "メンタルは睡眠と食事に強く相関します。低い日が続く場合は意識的な休息が必要です。",
            streak: "記録の連続日数は定着度を示します。途切れても気にせず「再開すること」を優先してください。",
            completion: "過去30日間の記録完了率です。80%以上をキープすることで精度の高いデータ分析が可能になります。",
            month_count: "今月記録を行った合計日数です。小さな積み重ねが目標達成への確実なプロセスとなります。"
        },
        en: {
            weight: "Weight fluctuates daily. Focus on the 7-day trend.",
            fat_p: "Body fat is affected by hydration. Measure consistently.",
            calories: "Today's estimated calories analyzed by AI.",
            sleep_h: "Aiming for ~7 hours stabilizes mental and physical health.",
            mental: "Mental condition correlates with sleep and diet.",
            streak: "Shows habit consistency. Restart immediately if broken.",
            completion: "Maintaining 80%+ enables highly accurate analysis.",
            month_count: "Total days logged this month."
        }
    };
    ...
    document.querySelectorAll('#settingMGrp .cond-btn').forEach(b => {
        b.addEventListener('click', () => {
            ...
            b.classList.add('active'); settingMVal = b.dataset.v;
        });
    });
    ...
    if (settingsBtn) {
        settingsBtn.addEventListener('click', async () => {
            ...
            if (tData) {
                ...
                if(sWeight) sWeight.value = ""; if(sFat) sFat.value = ""; if(sWake) sWake.value = "";
                document.querySelectorAll('#settingMGrp .cond-btn').forEach(b => b.classList.remove('active'));
                const defaultBtn = document.querySelector(`#settingMGrp .cond-btn[data-v="3"]`);
                if(defaultBtn) defaultBtn.classList.add('active'); settingMVal = 3;
            }
    ...
    if (btnSettingsSave) {
        btnSettingsSave.addEventListener('click', async () => {
            ...
            try {
                ...
                if (tWeightEl && tFatEl) {
                    ...
                    if (isNaN(tWeight) || isNaN(tFat)) { 
                        ...
                        btnSettingsSave.disabled = false; btnSettingsSave.innerText = "Save Changes"; return; 
                    }
                ...
                } else { tPayload.waketime = null; tPayload.sleep_hours = null; }
            ...
            } catch (err) { alert("エラーが発生しました: " + err.message); } finally { btnSettingsSave.disabled = false; btnSettingsSave.innerText = "Save Changes"; }
        });
    }
    ...
    function resetButtonUI(type) {
        ...
        if (type === 'wake') {
            ...
            if(btn) { btn.classList.remove('recorded'); if(icon) icon.className = 'fas fa-sun'; if(textSpan) textSpan.innerText = dict[currentLang].wake; }
        } else {
            btn = document.getElementById('btnBedtime'); icon = document.getElementById('bedIcon'); textSpan = document.getElementById('bedText');
            if(btn) { btn.classList.remove('recorded'); if(icon) icon.className = 'fas fa-moon'; if(textSpan) textSpan.innerText = dict[currentLang].sleep; }
        }
    }
    ...
    async function recordTime(type) {
        try {
            ...
            const { data: existing, error: fetchErr } = await supabaseClient.from('health_logs').select('id, waketime, bedtime').eq('user_id', user.id).eq('measured_date', logicalDateStr).maybeSingle();
            ...
            if (typeof window.loadDashboard === 'function') window.loadDashboard();
        } catch (e) { alert("保存エラーが発生しました: " + e.message); }
    }
    ...
    window.loadDashboard = async function() {
        try {
            ...
            resetButtonUI('wake'); resetButtonUI('bed');

            const { data: todayLog } = await supabaseClient.from('health_logs').select('waketime, bedtime').eq('user_id', user.id).eq('measured_date', logicalDateStr).maybeSingle();
            if (todayLog) {
            ...
            const { data: todayMeals } = await supabaseClient.from('meal_logs').select('calories').eq('user_id', user.id).eq('meal_date', logicalDateStr);
            let totalCal = 0;
            if (todayMeals) { totalCal = todayMeals.reduce((sum, meal) => sum + (Number(meal.calories) || 0), 0); }
            const calEl = document.getElementById('todayCalories');
            if (calEl) calEl.innerText = totalCal;

            const { data: recentLogs, error: logErr } = await supabaseClient.from('health_logs').select('*').eq('user_id', user.id).order('measured_date', { ascending: false }).limit(2);
            ...
            const { data: chartData, error: chartErr } = await supabaseClient.from('health_logs').select('measured_date, weight, body_fat, sleep_hours, mental_condition').eq('user_id', user.id).order('measured_date', { ascending: false }).limit(7);
            if (chartErr) throw chartErr;
            if (chartData) {
                globalChartLogs = chartData.reverse(); 
                if (document.getElementById('healthCorrelationChart')) renderDynamicChart();
            }
    ...
    function renderDynamicChart() {
        ...
        window.dashChart = new Chart(ctx, { type: 'bar', data: { labels, datasets }, options: { plugins: { legend: { display: false } }, layout: { padding: { top: 20 } }, scales } });
    }
    ...
    if (modeWeight && modeSleep) {
        modeWeight.addEventListener('click', () => { currentChartMode = 'weight'; modeWeight.classList.add('active'); modeSleep.classList.remove('active'); renderDynamicChart(); });
        modeSleep.addEventListener('click', () => { currentChartMode = 'sleep'; modeSleep.classList.add('active'); modeWeight.classList.remove('active'); renderDynamicChart(); });
    }
    ...
    if (btnMealOpen && mealModal) { btnMealOpen.addEventListener('click', () => { document.getElementById('quickMealDate').value = getLocalLogicalDateStr(new Date()); mealModal.style.display = 'flex'; }); }
    if (btnMealCancel && mealModal) { btnMealCancel.addEventListener('click', () => { mealModal.style.display = 'none'; }); }
    ...
    if (btnMealSave) {
        btnMealSave.addEventListener('click', async () => {
            ...
            if (!mealDate) return;
            btnMealSave.disabled = true; btnMealSave.innerText = fileInput.files.length > 0 ? dict[currentLang].msg_ai_analyzing : "Saving...";

            try {
                let imageUrl = null; let finalCalories = null;
                if (fileInput.files.length > 0) {
                    const file = fileInput.files[0];
                    imageUrl = await uploadCompressedImage(file, user.id);
                    try {
                        const response = await fetch('/api/analyze-meal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageUrl: imageUrl, memo: memo }) });
                        if (response.ok) {
                            const aiData = await response.json();
                            finalCalories = aiData.calories;
                            // ★ 【AI解説機能の実装】ここでAIのコメントを「Meals」ギャラリーのメモとして見れるように文章を追加しています
                            memo = memo ? `${memo}\n---\n[AI Analysis] ${aiData.analysis}` : `[AI Analysis] ${aiData.analysis}`;
                        }
                    } catch (aiInvokeError) { console.warn("AI Analysis skipped or failed:", aiInvokeError); }
                }

                const { error } = await supabaseClient.from('meal_logs').insert({
                    user_id: user.id, meal_date: mealDate, meal_type: type, content: memo, image_url: imageUrl, calories: finalCalories, created_at: new Date().toISOString()
                });
            ...
            } catch (err) { alert("Error: " + err.message); } finally { btnMealSave.disabled = false; btnMealSave.innerText = "Save"; }
        });
    }
    ...
    async function checkInitialSetup() {
        ...
        if (localFlag === 'true') { if (typeof window.loadDashboard === 'function') window.loadDashboard(); return; }
        ...
        if (!data || data.length === 0) { if (initialSetupModal) initialSetupModal.style.display = 'flex'; } else {
    ...
    if (initialSetupForm) {
        initialSetupForm.addEventListener('submit', async (e) => {
            ...
            try {
                const now = new Date(); const todayStr = getLocalLogicalDateStr(now);
                const yesterday = new Date(now.getTime()); yesterday.setDate(yesterday.getDate() - 1);
                const yesterdayStr = getLocalLogicalDateStr(yesterday);

                const btInput = document.getElementById('initBedtime'); const wtInput = document.getElementById('initWaketime');
                const wInput = document.getElementById('initWeight'); const fInput = document.getElementById('initFat');

                const btVal = btInput ? btInput.value : null; const wtVal = wtInput ? wtInput.value : null;
                const wVal = wInput ? wInput.value : null; const fVal = fInput ? fInput.value : null;
                ...
                let sleepHours = null;
                if(wDate && bDate) { sleepHours = parseFloat(((wDate - bDate) / 3600000).toFixed(1)); if (sleepHours < 0) sleepHours += 24; }
